...
public class DurabilityHUD {
    public static final Identifier DURABILITY_HUD_LAYER = Identifier.fromNamespaceAndPath(A3Utils.MOD_ID, "durability-hud-layer");

    private static final int equipment_slots[] = {EquipmentSlot.FEET.getIndex(36), EquipmentSlot.LEGS.getIndex(36), EquipmentSlot.CHEST.getIndex(36), EquipmentSlot.HEAD.getIndex(36)};
    ...
    private static void renderArmorBars(GuiGraphics context, int x, int y) {
        for (int slot : equipment_slots) { renderBar(context, A3UtilsClient.mc.player.getInventory().getItem(slot), x, y -= 3); }
    }
