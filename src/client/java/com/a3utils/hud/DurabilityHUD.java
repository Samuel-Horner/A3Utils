...
public class DurabilityHUD {
    ...
    private static void drawQuad(DrawContext context, int x, int y, int width, int height, int color) { context.fill(x, y, x + width, y - height, color); }
    ...
    private static void renderBar(DrawContext context, ItemStack stack, int x, int y) {
        ...
        drawQuad(context, x, y, 13, 2, 0xFF000000);
        drawQuad(context, x, y - 1, stack.getItemBarStep(), 1, ColorHelper.fullAlpha(stack.getItemBarColor()));
    }

    private static void renderArmorBars(DrawContext context, int x, int y) {
        for (int slot : equipment_slots) { renderBar(context, mc.player.getInventory().getStack(slot), x, y -= 3); }
    }
    ...
    private static void renderArmorArea(DrawContext context) {
        ...
        if (mc.player.experienceLevel > 0) { y -= 7; }
